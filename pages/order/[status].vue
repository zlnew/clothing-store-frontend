...
definePageMeta({
  ...
  validate: async ({ params }) => {
    ...
    return (typeof params.status === 'string') && statuses.includes(params.status)
  }
})
...
const statusButtons = [
  { to: '/order/active', label: 'Active', key: 'active' },
  { to: '/order/cancelled', label: 'Cancelled', key: 'cancelled' },
  { to: '/order/finished', label: 'Finished', key: 'finished' }
]
...
<template>
  <UContainer :ui="{ constrained: 'max-w-4xl' }">
    <div class="my-8 md:my-14">
      <div class="space-y-4">
        <UBreadcrumb :links="breadcrumbsLinks" />
